...
class Tutorial2: SKScene, SKPhysicsContactDelegate {
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        case .StageClear:
            if stageClearDoneFlag == false {
                ...
                gridNode.resetSquareArray(color: "blue")
                if Tutorial.tutorialPhase == 3 {
                    createTutorialLabel(text: "やった!!", posY: 1100)
                    createTutorialLabel(text: "全部の敵をたおした!", posY: 1040)
                    buttonNext.state = .msButtonNodeStateActive
                    buttonAgain.state = .msButtonNodeStateActive
                } else {
                    createTutorialLabel(text: "グレイト!!", posY: 1100)
                    createTutorialLabel(text: "これで、じゅんび完了だ!", posY: 1040)
    ...
    func tutorialFlow() {
        ...
        case 3:
            ...
            case .T1:
                self.createTutorialLabel(text: "やつらがまたやってきた！！", posY: 880)
                self.createTutorialLabel(text: "全部の敵をたおせ！", posY: 820)
        ...
        case 4:
            switch tutorialState {
            case .T1:
                self.createTutorialLabel(text: "次は、アイテムを使ってみよう！", posY: 880)
                self.createTutorialLabel(text: "時限ばくだんをとるんだ！", posY: 820)
                self.gridNode.showMoveArea(posX: activeHero.positionX, posY: activeHero.positionY, moveLevel: activeHero.moveLevel)
            ...
            case .T3:
                ...
                self.createTutorialLabel(text: "いいぞ!", posY: 880)
                self.createTutorialLabel(text: "時限ばくだんをゲットした！", posY: 820)
                showTouchScreen(waitTime: 2.0)
                break;
            case .T4:
                resetDiscription()
                ...
                self.createTutorialLabel(text: "アイテムボタンをタッチしよう!", posY: 820)
            ...
            case .T5:
                resetDiscription()
                self.createTutorialLabel(text: "時限ばくだんのアイコンをタッチ!", posY: 820)
            ...
            case .T6:
                resetDiscription()
                self.createTutorialLabel(text: "紫色のところをタッチすれば", posY: 880)
                self.createTutorialLabel(text: "時限ばくだんをしかけられるぞ", posY: 820)
                showTouchScreen(waitTime: 2.0)
                gridNode.showtimeBombSettingArea()
                break;
            case .T7:
                resetDiscription()
                self.createTutorialLabel2(text: "ここに時限ばくだんをしかけよう!!",posX: self.size.width/2, posY: 900, color: UIColor.white, size: 35)
            ...
            case .T8:
                resetDiscription()
                self.createTutorialLabel(text: "時限ばくだんは次のターンにばくはつするぞ!", posY: 880)
                showTouchScreen(waitTime: 2.0)
                break;
            case .T9:
                resetDiscription()
                self.createTutorialLabel(text: "グレイト!", posY: 880)
                self.createTutorialLabel(text: "時限ばくだんで敵をはかいした!", posY: 820)
                showTouchScreen(waitTime: 2.0)
                break;
            case .T10:
                resetDiscription()
                self.createTutorialLabel(text: "ゲームが進めば", posY: 880)
                self.createTutorialLabel(text: "もっと多くのアイテムが出てくるぞ！", posY: 820)
                showTouchScreen(waitTime: 2.0)
                break;
            case .T11:
                resetDiscription()
                self.createTutorialLabel(text: "残りの敵をたおそう！！", posY: 880)
                showTouchScreen(waitTime: 2.0)
                break;
            default:
                break;
