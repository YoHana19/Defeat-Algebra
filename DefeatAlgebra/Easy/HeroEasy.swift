...
class HeroEasy: SKSpriteNode {
    ...
    func heroSingleMove() {
        ...
        case .front:
            /* Move hero backward */
            let move = SKAction.moveBy(x: 0, y: -CGFloat(gameSceneEasy.gridNode.cellHeight), duration: self.moveSpeed)
            self.run(move)
            break;
        case .back:
            /* Move hero forward */
        ...
        case .left:
            /* Move hero left */
            let move = SKAction.moveBy(x: -CGFloat(gameSceneEasy.gridNode.cellWidth), y: 0, duration: self.moveSpeed)
            self.run(move)
            break;
        case .right:
            /* Move hero right */
