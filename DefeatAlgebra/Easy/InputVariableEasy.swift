...
class InputVariableEasy: SKSpriteNode {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if nodeAtPoint.name == "button1" {
            inputDone(value: 1)
            gameSceneEasy.xValue = 1
            for enemy in gameSceneEasy.gridNode.enemyArray {
                enemy.calculatePunchLength(value: gameSceneEasy.xValue)
            }
            if MainMenu.soundOnFlag {
                ...
                let flash = SKAction.run({
                    gameSceneEasy.gridNode.flashGridForCane(labelNode: gameSceneEasy.valueOfX, numOfFlash: 1)
                    gameSceneEasy.caneOnFlag = true
                })
        ...
        if nodeAtPoint.name == "button2" {
            ...
            if MainMenu.soundOnFlag {
                ...
                let wait = SKAction.wait(forDuration: 3.0)
                let flash = SKAction.run({
                    gameSceneEasy.gridNode.flashGridForCane(labelNode: gameSceneEasy.valueOfX, numOfFlash: 2)
                    gameSceneEasy.caneOnFlag = true
                })
                let seq = SKAction.sequence([wait, flash])
                self.run(seq)
        ...
        if nodeAtPoint.name == "button3" {
            ...
            if MainMenu.soundOnFlag {
                ...
                let wait = SKAction.wait(forDuration: 3.0)
                let flash = SKAction.run({
                    gameSceneEasy.gridNode.flashGridForCane(labelNode: gameSceneEasy.valueOfX, numOfFlash: 3)
                    gameSceneEasy.caneOnFlag = true
                })
                let seq = SKAction.sequence([wait, flash])
                self.run(seq)
