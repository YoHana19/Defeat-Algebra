...
class PauseScreenEasy: SKSpriteNode {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if nodeAtPoint.name == "pauseResume" {
            gameSceneEasy.pauseFlag = false
            self.isHidden = true
            
        } else if nodeAtPoint.name == "pauseRetry" {
            /* Sound */
            gameSceneEasy.main.stop()
            gameSceneEasy.stageClear.stop()
            
            /* Grab reference to the SpriteKit view */
            let skView = gameSceneEasy.view as SKView!
