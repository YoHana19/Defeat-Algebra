...
class EnemyEasy: SKSpriteNode {
    ...
    func punchAndMove() {
        ...
        let gridNode = self.parent as! GridEasy
        /* Get GameSceneEasy */
        let gameSceneEasy = gridNode.parent as! GameSceneEasy
        ...
        if self.positionY < self.valueOfEnemy {
            ...
            let decreseLife = SKAction.run({
                if self.wallHitFlag == false {
                    gameSceneEasy.life -= 1
                    gameSceneEasy.setLife(numOflife: gameSceneEasy.life)
                } else {
                    gameSceneEasy.life += 0
                }
            })
            ...
            let moveNextEnemy = SKAction.run({
                self.myTurnFlag = false
                /* For sound */
                gameSceneEasy.hitCastleWallSoundDone = false
                if gridNode.turnIndex < gridNode.enemyArray.count-1 {
                    gridNode.turnIndex += 1
                    gridNode.enemyArray[gridNode.turnIndex].myTurnFlag = true
                }
    ...
    func punchToCastle() {
        ...
        /* Set punchLength */
        self.punchLength = self.position.y+gameSceneEasy.gridNode.position.y-gameSceneEasy.castleNode.position.y-40+5
    ...
    func resetColorizeEnemy(color: UIColor) {
        self.run(SKAction.colorize(with: color, colorBlendFactor: 0, duration: 0.50))
    }
}
